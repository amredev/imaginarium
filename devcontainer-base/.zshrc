...
export HISTFILE="$HOME/persisted/shell-history"
...
ZSH_THEME_AWS_PROFILE_PREFIX="%F{magenta}("
ZSH_THEME_AWS_PROFILE_SUFFIX=")%f"
ZSH_THEME_AWS_REGION_PREFIX="%F{magenta}["
ZSH_THEME_AWS_REGION_SUFFIX="]%f"

### ====================

# Source a custom .zshrc that needs to run before the oh-my-zsh.sh script
if [[ -f "$HOME/.zshrc.custom.pre.sh" ]] ; then
    # shellcheck disable=SC1091
    . "$HOME/.zshrc.custom.pre.sh"
fi
...
# This info is a bit excessive. VSCode already has a good git integration
...
# Source a custom .zshrc that needs to run after the oh-my-zsh.sh script
if [[ -f "$HOME/.zshrc.custom.post.sh" ]] ; then
    # shellcheck disable=SC1091
    . "$HOME/.zshrc.custom.post.sh"
fi
